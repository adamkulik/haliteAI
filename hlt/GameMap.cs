...
public class GameMap {
    ...
    /// <param name="b"></param>
    /// <returns></returns>
    public List<Entity> GetObstaclesBetween (Ship ship, Position target, int ownerId, bool settle) {
        var obstacles = new List<Entity> ();
        foreach (var planet in planets) {
            if (planet.EntityInfo.Owner == ownerId && !settle)
            {
                if (Collision.IntersectSegmentCircle(ship, target, planet, Constants.ShipRadius + 4))
                    obstacles.Add(planet);
            }
            else
            {
                if (Collision.IntersectSegmentCircle(ship, target, planet, Constants.ShipRadius + 0.3))
                    obstacles.Add(planet);
            }
        }
        foreach(var othership in ships)
        {
            if (Collision.IntersectSegmentCircle(ship, target, othership, Constants.ShipRadius + 2))
                {
                obstacles.Add(othership);
            }
        }

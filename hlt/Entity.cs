...
public class Entity {
    ...
    public Position GetClosestPointToEntity (Entity target, double minimumDistance = 3) {
        ...
        return new Position ((((random.NextDouble() - 0.5) * 0.5 * minimumDistance) + target.Position.X) + radius  * Math.Cos (angle), (((random.NextDouble() - 0.5) * 0.5 * minimumDistance) + target.Position.Y) + radius * Math.Sin (angle));
    }
...
public class Ship : Entity {
    ...
    public Ship (int owner, Queue<String> tokens) : base () {
        ...
        this.Position = new Position (Double.Parse (tokens.Dequeue (), CultureInfo.InvariantCulture), Double.Parse (tokens.Dequeue (), CultureInfo.InvariantCulture));
        this.health = int.Parse (tokens.Dequeue ());
        this.velocity = new Velocity (Double.Parse (tokens.Dequeue (), CultureInfo.InvariantCulture), Double.Parse (tokens.Dequeue (), CultureInfo.InvariantCulture));
    ...
    /// <returns></returns>
    public string Navigate(Position target, GameMap map, Double speed, bool avoidObstacles = true, int maxCorrection = 90, bool settle = true) {
        if (maxCorrection <= 0)
            return string.Empty;
        var distance = this.GetDistance (target);
        var angle = this.GetAngle (target);
        angle *= Math.PI / 180;
        var obstacles = map.GetObstaclesBetween (this, target, this.EntityInfo.Owner, settle);
        if (avoidObstacles == true && obstacles.Count > 0) {
            Log.Information ("Avoiding Obstacles", LogingLevel.User);
            var newTargetDX = Math.Cos (angle + Math.PI/180) * distance;
            var newTargetDY = Math.Sin (angle + Math.PI/180) * distance;
        ...
        speed = distance >= speed ? speed : distance - 1;
        angle *= 180 / Math.PI;
        return this.Move (Convert.ToInt32 (speed), Convert.ToInt32 (angle));
    }
    public string newNavigate(Position target, GameMap map, Double speed, bool avoidObstacles = true, int maxCorrection = 90, bool settle = true)
    {
        if (maxCorrection <= 0)
            return string.Empty;
        var distance = this.GetDistance(target);
        var angle = this.GetAngle(target);
        angle *= Math.PI / 180;
        double newX = ((this.Position.X - this.Position.X )*Math.Cos(angle)) + (((this.Position.Y + speed) - this.Position.Y) * Math.Sin(angle)) + (this.Position.X);
        double newY = ((this.Position.X - this.Position.X) * Math.Sin(angle)) + (((this.Position.Y + speed) - this.Position.Y) * Math.Cos(angle)) + (this.Position.Y + speed);
        Position newTarget = new Position(this.Position.X, this.Position.Y + 7);
        var obstacles = map.GetObstaclesBetween(this, newTarget,this.EntityInfo.Owner, settle);
        if (avoidObstacles == true && obstacles.Count > 0)
        {
            obstacles.Sort(delegate (Entity a, Entity b)
            {
                if (this.GetDistance(a.Position) < this.GetDistance(b.Position)) return -1;
                else if (this.GetDistance(a.Position) == this.GetDistance(b.Position)) return 0;
                else return 1;
            });
           ...
           /* Log.Information("Avoiding Obstacles", LogingLevel.User);
            ...
            var newTargetDY = Math.Sin(angle + Math.PI / 180) * distance;
            var newTarget = new Position(this.Position.X + newTargetDX, this.Position.Y + newTargetDY);
            return this.newNavigate(newTarget, map, speed, true, maxCorrection - 1);*/
